...
@interface ShoesViewCell()



@end

@implementation ShoesViewCell
...
-(void)setUpCellWithDict:(NSDictionary *)dict{
    ...
    self.ShoesTitleLab.text=dict[@"goods_title"];
    
    self.ShoesPriceLab.text=dict[@"goods_price"];
    ...
    if ([manager fileExistsAtPath:path]) {
        //图片已存在的话调用沙盒图片
        self.ShoesImg.image=[UIImage imageNamed:path];
        return;
